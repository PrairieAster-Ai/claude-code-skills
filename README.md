...
| [**dev-onboarding**](skills/dev-onboarding/) | Fresh clone → running localhost, fast + self-diagnosing. A read-only `doctor` (missing tools/versions/secrets → exact fixes) + idempotent `bootstrap`, for two paths: internal **team** vs. external **contributor** (no access). Config-driven; composes with the Getting-Started page. | `/dev-onboarding [doctor\|bootstrap\|scaffold\|verify]` |
