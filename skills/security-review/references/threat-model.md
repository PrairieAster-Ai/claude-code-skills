...
**Attack:** A PR adds `.semgrep.yml`, `.gitleaks.toml`, `.bandit`, `pyproject.toml [tool.bandit]`, etc. that disables rules, excludes paths, or extends from an attacker-controlled URL.
...
- Detect changes to `.semgrep.yml`, `.gitleaks.toml`, `.bandit`, etc. in the diff and flag them as a finding category of their own ("PR modifies security tool config — review separately").
